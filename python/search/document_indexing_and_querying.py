...
DOCUMENTS_FOLDER = 'D:\doc'  # Replace with the folder path containing your text files
...
def index():
    ...
    if request.method == 'POST':
        ...
        if not query:
            return render_template('index.html', results=[], error="Please enter a valid query.")
        if not doc_vectors or not vocab:
            return render_template('index.html', results=[],
                                   error="No files are indexed. Please check the document folder.")
...
if __name__ == '__main__':
    # Load and process documents from the folder before starting the server
    print("Loading and indexing documents...")
    docs, filenames = load_documents(DOCUMENTS_FOLDER)
    tf_list, idf = build_indices(docs)
